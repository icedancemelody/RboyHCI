...
def send_audio(ws):
    ...
    # with open("../python/wav/"+pcm_file, 'rb') as f:
    #     pcm = f.read()
...
def on_message(ws, message):
    ...
    logger.info("miao")
    logger.info(message)
    result=message.split("\"")[13]

    if result=="sn":
        return
    '''
    sentence_result=message["result"]
    ...
    #     f.write(result+"\n")
    with open("../python/result.txt",'a') as f:
        f.write(result+"\n")
    f.close()

    # 结果加入全局词语结果队列,
    # globalResultWords.addWord(result)
